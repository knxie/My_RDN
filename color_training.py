...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun 25 14:39:05 2019
...
parser.add_argument("--loadFile", type=str, default="logs//exp1//netB50color_99.pth", help="Model location")
...
parser.add_argument("--outf", type=str, default="logs//exp1", help='path of log files')
...
parser.add_argument("--channels",type=int,default=1, help= "1-the gray, 2-the color")
...
def main():
    ...
    dataset_val = Dataset(train=True)
    ...
    # Load model
    if opt.loadModel:
       modelLocate = opt.loadFile
       net = load_GPUS(net, modelLocate)
       beginEpochs  = int(modelLocate[modelLocate.rindex('_')+1:modelLocate.rindex('.')])
    ...
    step = 0
    noiseL_B=[15,40] # ingnored when opt.mode=='S'
    for epoch in range(beginEpochs,opt.epochs):
        if epoch <20:
            current_lr = opt.lr
        elif epoch < 30:
